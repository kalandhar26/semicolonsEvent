...
function StatCard({ label, value, unit, color, sub }) {
  return (
    ...
    >
      ...
      >
        {value}
        <span style={{ fontSize: 14, color: C.muted, marginLeft: 3 }}>{unit}</span>
      </span>
...
function CircuitLight({ state }) {
  const cfg = {
    ...
    HALF_OPEN: {
      color: C.amber,
      label: "HALF-OPEN",
      sub: "Probing — test requests only",
    },
  }[state];
      ...
      >
        ...
        {events.map((e, i) => (
          <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: 12 }}>
            <span style={{ color: colorFor(e.type), fontFamily: "monospace", flexShrink: 0, fontSize: 13 }}>
              {iconFor(e.type)}
            </span>
...
export default function AIPredictorDashboard() {
  ...
  const [events, setEvents] = useState([
    { type: "info", time: now(), msg: "System online. Monitoring Order Service." },
  ]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const errorRate = targetErr;
      const latency = targetLat;

      const score = isOpen ? computeAnomaly(2) : computeAnomaly(errorRate);
      setAnomaly(score);
      setRequestCount(t * 12);

      setData((prev) => [
        ...prev.slice(-HISTORY + 1),
        makePoint(t, errorRate, latency),
      ]);
      ...
      if (score > ANOMALY_THRESHOLD && circuitRef.current === "CLOSED") {
        ...
        addEvent(
          "anomaly",
          `Anomaly score ${Math.round(score * 100)} > 75 — circuit breaker OPENED. Traffic blocked.`
        );
        setHealCountdown(10);
      }
    }, 800);
    ...
    >
      ...
      >
        <div>
          ...
          >
            Persistent Semicolon Hackathon · Order Service
          </div>
          <h1
            style={{
              margin: 0,
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button
            ...
            onMouseEnter={(e) => {
              if (!failing) e.currentTarget.style.background = C.redDim;
            }}
            onMouseLeave={(e) => {
              if (!failing) e.currentTarget.style.background = "transparent";
            }}
          >
            {failing ? (
              <>
